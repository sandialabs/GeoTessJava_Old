...
import gov.sandia.gmp.util.containers.hash.maps.HashMapIntegerDouble.Entry;
...
public class GeoTessModelTest
{
	 ...
	 */
	private static GeoTessModel model;

	/**
	 * resources/permanent_files/smallModel.geotess
	 */
	private static GeoTessModel small_model;

	private static GeoTessModel model2d;

	@BeforeClass
	public static void setUpBeforeClass() throws Exception
	{
		System.out.println("GeoTessModelTest");
...
//	}
	...
	public void testSetActiveRegion3DLayers() throws GeoTessException, IOException
	{
		...
		double polygonRadius = VectorUnit.angleDegrees(polygonCenter, polygon.getPoint(0));

//		System.out.println(VectorGeo.getLatLonString(polygonCenter));
//		System.out.println(VectorGeo.getLatLonString(polygon.getPoint(0)));
//		System.out.println(polygonRadius);
		...
		for (int i=0; i<small_model.getPointMap().size(); ++i)
		{
			...
			boolean ok = VectorUnit.angleDegrees(polygonCenter, point) < polygonRadius 
					&& small_model.getPointMap().getLayerIndex(i) >= bottomLayer 
					&& small_model.getPointMap().getLayerIndex(i) <= topLayer;

					if (!ok)
						System.out.printf("%nlayer=%d polygonRadius = %f  distance = %f%n%n",
								small_model.getPointMap().getLayerIndex(i),
								polygonRadius, VectorUnit.angleDegrees(polygonCenter, point));
		...
		{
		  boolean ok;
			double distance = VectorUnit.angleDegrees(polygonCenter, small_model.getGrid().getVertex(vertex));
			...
			for (int layer=0; layer<small_model.getNLayers(); ++layer)
			{
				...
				{
					if (profile.getPointIndex(n) < 0)
						ok = distance >= polygonRadius || layer < bottomLayer || layer > topLayer;
					else
						ok = distance <= polygonRadius && layer >= bottomLayer && layer <= topLayer;
						
					if (!ok)
					{
					  System.out.printf("pointIndex=%d, polygonRadius=%1.4f, distance=%1.4f, layer=%d%n", 
					      profile.getPointIndex(n), polygonRadius, distance, layer);
					}
					assertTrue(ok);
				}
			}
